...
def get_diff(base_path,save_path,dist_thres,num,num_total):
    ...
    img_feature_set=np.load(os.path.join(base_path,'img_siam_feature_norm_%d.npy'%(num)))
    pc_feature_set=np.load(os.path.join(base_path,'pc_siam_feature_norm_%d.npy'%(num)))
...
if __name__=='__main__':
    ...
    base_path='./siam_kitti_test14_result_all_dist_thres_%0.2f_pos_margin_%0.2f_neg_margin_%0.2f/'%(args.train_dist_thres,args.pos_margin,args.neg_margin)

    num_total=len(os.listdir(base_path))//10
    save_path='./siam_result14_error_%0.2f_%0.2f_pos_margin_%0.2f_neg_margin_%0.2f/'%(args.train_dist_thres,args.test_dist_thres,args.pos_margin,args.neg_margin)
    try:
        os.mkdir(save_path)
    except:
        pass
